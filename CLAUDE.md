...
- `npm run check` — runs `npm test` then `npm run typecheck` (`npm test && npm run typecheck`), the two checks required before handing control back.
- `npm run tdd` — mocha watch mode.
- Run a single test file: `npx mocha src/path/to/thing.test.ts`. Filter by name with `--grep "<pattern>"`.
- Throwaway/repro files: the mocha glob (`src/**/*.test.ts`) sweeps in **any** matching file, tracked or not. Name scratch repros `*.scratch.test.ts` — that suffix is in mocha's `ignore` list so it won't be picked up by `npm test`. To run one in isolation you must bypass the package.json mocha config (its `spec` otherwise unions the whole suite back in): `npx mocha --no-package --require ts-node/register/transpile-only src/path/to/thing.scratch.test.ts`.
...
**Before handing control back to the user, always run BOTH `npm test` and `npm run typecheck`** when you've touched any code — tests no longer type-check, so green tests alone can hide a compile error. Run them with `npm run check` (which is `npm test && npm run typecheck`), or run both in parallel in a single batch of tool calls. Report both results.
...
3. `src/run/runSubgraph.ts` + `src/run/runVertex.ts` — construct the RxJS pipeline. Each vertex runs its operations (`computeFromFields`, `load*`, `reaction*`, …) via `operationsByVertexId`. Subgraphs short-circuit when neither the vertex's Redux slice, a tracked upstream field, nor a tracked action changed (the `shouldRun` gate in `runSubgraph`). This is the mechanism that prevents unnecessary recomputation/re-render.
