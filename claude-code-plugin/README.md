...
| `verdux:react-integration` | Module-singleton graph; `GraphContext`; Suspense-first `useVertexState` hook (via `observable-hooks`); inline `graph.dispatch` (with `useGraph`, never `useDispatch`); in-band sentinels for empty/error states; fine-grained per-leaf picks |
...
directly invocable as `/verdux:graph-design`, etc.
